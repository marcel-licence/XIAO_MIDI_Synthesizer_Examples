//
// Modified copy from AuditionMode.h
//

#ifndef MIDIPLAYERMODE_H
#define MIDIPLAYERMODE_H
...
// MidiPlayerMode Mode 1 (default)
// MidiPlayerMode is a derived class from State that represents a specific state in the state machine.
...
class MidiPlayerMode : public State
{
public:
    enum {ID = 4}; // Unique ID for this state.

    MidiPlayerMode(); // Constructor that initializes the state.
